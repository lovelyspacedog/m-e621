...
- **u18chan** (SiteMode `"u18chan"`) uses dedicated routes (`/#/u18chan/:board`, `/#/u18chan/:board/thread/:id`) with HTML-scrape proxies (`/api/u18chan/*`). Sidebar lists Indices catalogs only (Cub never wired; Gore behind `u18chanIncludeGore`, default off). Guest posting identity in Settings; thread replies use native compose — not `PostCommentsPanel`. Thread page toggles Thread / Gallery / Scroll; dump pages come from `extractU18chanImageDump` (leading same-name-as-OP image posts).
