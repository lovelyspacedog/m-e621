...
- **News** — merged furry news from Flayrah and Dogpatch Press via public RSS (Flayrah full feed plus curated taxonomy feeds; Dogpatch site feed), in-app article reader, archive fallback for deep links, read/saved state, magazine or list layout, source filter chips, and attributed links back to each origin. Not a Federated child; no login, comments, or ratings. Flayrah is work-safe oriented; Dogpatch may include adult or investigative topics (no extra filter in PawFeed). Last-good feed cache remains available offline.
