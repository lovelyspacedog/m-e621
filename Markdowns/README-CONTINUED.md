...
- **Murrtube** / **Badpups** (XTRA) — optional adult video sites. Enable **XTRA mode** in Post settings (off by default). They appear in the site picker only when XTRA is on and **SFW only** is off; turning SFW on or XTRA off demotes an active XTRA mode to e621. Not Federated children; no Saved bookmarks, login, or favorites in v1. Murrtube uses an age-gated Inertia proxy (`/api/murrtube`); Badpups uses HTML/JSON-LD plus Bunny CDN MP4 via `/api/badpups` and `/api/download`. Feed cards size to each video’s thumbnail aspect ratio (landscape stays landscape).
...
- Available on every mode except Tailspace and News
...
- **SFW only** (Post settings, app-bar shield, and Site block in the sidebar) forces safe rating on adapters that support it, overrides conflicting rating search tags, and hides non-safe posts in list feeds. On e621, e6ai, Furbooru, FurAffinity, and Federated, adding `rating:safe` or `rating:s` to the search turns SFW on (removing that tag does not turn it off; excludes like `-rating:safe` do not turn it on). Federated no longer snackbars when SFW’s injected safe rating is dropped on children that lack rating search (e.g. SoFurry). Direct post URLs still open. Off by default; global across site modes (not per-profile). While SFW only is on, XTRA video sites are removed from the picker and an active XTRA mode demotes to e621.
- **XTRA mode** (Post settings) adds Murrtube and Badpups to the site list. Off by default; persisted in settings backups / host sync with the rest of `misc`.
