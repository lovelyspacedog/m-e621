...
- **Video** (optional) — adult video hub for Murrtube and Badpups. Enable **Video mode** in Post settings (off by default). Appears in the site picker only when Video mode is on and **SFW only** is off; turning SFW on or Video off demotes Video to e621. Not a Federated child; no Saved bookmarks, login, or favorites in v1. Use **Sites in this search** (or landing chips while Video is active) to include Murrtube and/or Badpups; both default on and date-merge. Murrtube uses an age-gated Inertia proxy (`/api/murrtube`); Badpups uses HTML/JSON-LD plus Bunny CDN MP4 via `/api/badpups` and `/api/download`. Feed cards size to each video’s thumbnail aspect ratio (landscape stays landscape). The site chip shows each clip’s title (truncated). Layout → Autoplay video is Video-only and independent of Media → Autoplay video in feed.
