...
Tailspace posts, Flayrah, and Local are not included in the Federated Posts feed. Tailspace comics can appear in Federated Pools name browse when **Sites in Pools → Tailspace comics** is on. When the browser is offline, remote gallery modes are disabled; Local and Flayrah (last-good RSS cache) remain available.
...
- **SFW only** (Post settings, app-bar shield, and sidebar switch) forces safe rating on adapters that support it, overrides conflicting rating search tags, and hides non-safe posts in list feeds. Direct post URLs still open. Off by default; global across site modes (not per-profile).
