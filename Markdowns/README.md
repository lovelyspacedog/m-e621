...
- **Flexible feeds** — switch between full-width lists, thumbnail grids, and compact cards; toggle infinite scroll or page with buttons; rich filtering and media controls. Global **SFW only** (toolbar / sidebar / Post settings) forces safe rating across sites and hides non-safe posts in list feeds; typing `rating:safe` (or `rating:s`) on e621-family / Furbooru / FurAffinity / Federated turns it on.
