...
- **Federated browsing** — date-merge child sites into a Search or Following feed; landing chips multi-select inclusion; Defaults / Auth-only presets, per-site filters, origin-aware actions, and e621/e6ai Pools browse/reader plus Inkbunny pools via watch / open-by-id / chips (optional Tailspace comics in name browse).
...
- **Powerful browsing tools** — browse and watch pools (e621-family, Inkbunny, and Federated merge, with new-page badges), organize starred tags and saved searches, use the multi-site post suggester and favorite analyzer, and explore artist dashboards (heatmap, top posts, tag ranks).
