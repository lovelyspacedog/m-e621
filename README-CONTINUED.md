...
- The landing page uses site-mode chips, a primary Browse posts action, outlined Scent Marks / Changelog & TOS actions, a capability summary, a random [Furry Dictionary](https://the-furry-dictionary.avoonix.com/) slang snippet each visit, honest About/limits copy, dual Tony Pup / Avoonix Latest updates commit columns, a Changelog & TOS dialog (Changelog / TOS tabs), and an AGPL/age footer. The TOS tab lists fair-use summaries for supported sites (Tailspace omitted; no public TOS) plus Fluffle, each with a link to the official document. The hero tagline and About copy mention a local folder only when Local browse is available (Chromium File System Access or the Tauri app).
