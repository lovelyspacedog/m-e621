...
- **View transitions:** `router.beforeResolve` must not start a transition on first load (`!from.name`). Doing so captures an empty shell and Posts looks stuck. Also skip when leaving `LandingPage` — named `applogo`/`tagsearch` + router-view `out-in` can capture an empty `v-main` and blank Scent Marks / Browse posts.
