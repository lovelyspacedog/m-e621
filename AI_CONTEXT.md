...
**m-e621** (`package.json` name `m-e621`) is a personal, AI-assisted fork of [Material e621](https://github.com/avoonix/material-e621). It is a Vue 3 SPA that browses multiple furry imageboards and a local media folder from one UI.
...
- Tailspace browsing uses dedicated routes (`/tailspace/...`), not the e621-shaped Posts/Pools/Suggester/Analyzer/Dashboard pages. Router guards enforce this.
...
- User-Agent / `_client` query: `m-e621/<git>` (`src/worker/api/index.ts`). Fluffle UA: `m-e621/1.0 (by lovelyspacedog on GitHub)`.
...
- **SoFurry / other non-e621 modes must not fall through to e621 comments, notes, pools, suggester, analyzer, or dashboard.**
...
- **View transitions:** `router.beforeResolve` must not start a transition on first load (`!from.name`). Doing so captures an empty shell and Posts looks stuck.
