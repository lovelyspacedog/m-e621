...
export const changelogSections: ChangelogSection[] = [
  {
    ...
    items: [
      ...
      "Posts browser Back / ?page= reloads the feed and resets Federated merge leftovers",
      "Pools: ?chunk= tracks the fullscreen post; chunk changes clear stale ?post=; resume works with ?chunk= alone",
      "Fullscreen exit scroll and Federated media switches use postFeedKey (not bare numeric ids)",
      "Mode redirects keep search query params; Federated stars:me remaps to favs:me on non-Itaku children",
      "Furbooru write/read paths reject HTML error pages before JSON parse; Federated/Local tag autocomplete no longer hits e621",
      "Federated Favorite Analyzer errors when no children are enabled or the favorite sample is empty",
      "Local Tauri MP4 listing sniffs ftyp (mislabeled MPEG-TS marked unplayable); sidecar tag edits use the merge lock",
      "Pool covers keyed by origin:postId; Flayrah archive articles persist offline and reject wrong node ids",
      "Scent Marks admin auth/pin/delete are rate-limited; client IP uses X-Real-IP/peer (not spoofable XFF)",
      "Vite download/Fluffle proxies send Referer for Weasyl/Itaku/SoFurry; bare sofurryfiles.com allowlisted",
      "Settings restore rejects non-object JSON; preview runs migrations on a clone; scent unlock re-checks /auth",
      "Sidebar Suggestions honor sidebarSuggestionLimit; Automatic data saver picks low when offline or type=none",
      "Single-site posts stamp __meta.originMode; favorite/unfavorite refuse Flayrah/Tailspace chrome",
      "FA download cookies prefer X-FA-Cookies / env over ?fa=; Vite returns 501 for Scent Marks with a serve.py hint",
      "Pool fullscreen advances include blacklisted pages; backward chunk sync no longer races; scroll reaches gap placeholders",
      "Save-all reports unavailable gap counts; watched pool badges use lastSeenUpdatedAt when post_count is unchanged",
      "40-tag truncation snackbar fires once per search (not on every page-1 reload)",
